...
import { getAgentMentionDirectory, extractAgentMentions } from '../messaging/shared/agent-mentions';
...
export function createFeishuReplyDispatcher(params) {
    ...
    const deliveredReplyTexts = [];
    const deliveredHandoffTexts = [];
    const deliveredMentionTargets = new Map();
    // ---- Build dispatcher ----
    const { dispatcher, replyOptions, markDispatchIdle } = core.channel.reply.createReplyDispatcherWithTyping({
        ...
        deliver: async (payload) => {
            ...
            const mentionResolution = chatType === 'group'
                ? extractAgentMentions({
                    text,
                    directory: await getAgentMentionDirectory(LarkClient.globalConfig ?? cfg),
                    currentAgentId: agentId,
                    currentAccountId: accountId,
                })
                : { cleanedText: text, targets: [] };
            deliveredReplyTexts.push(text);
            deliveredHandoffTexts.push(mentionResolution.targets.length > 0 ? mentionResolution.cleanedText : text);
            for (const target of mentionResolution.targets) {
                deliveredMentionTargets.set(target.agentId, target);
            }
            const sendableMentionTargets = mentionResolution.targets
                .filter((target) => target.botOpenId)
                .map((target) => ({ openId: target.botOpenId, name: target.displayName }));
            const outgoingText = sendableMentionTargets.length > 0 ? mentionResolution.cleanedText : text;
            const chunksForText = (input) => {
                const chunks = shouldUseCard(input)
                    ? core.channel.text.chunkTextWithMode(input, textChunkLimit, chunkMode)
                    : core.channel.text.chunkTextWithMode(core.channel.text.convertMarkdownTables(input, tableMode), textChunkLimit, chunkMode);
                if (chunks.length === 0 && sendableMentionTargets.length > 0) {
                    return [''];
                }
                return chunks;
            };
            // ---- Static delivery ----
            if (shouldUseCard(outgoingText)) {
                const chunks = chunksForText(outgoingText);
                ...
                for (const chunk of chunks) {
                    try {
                        await sendMarkdownCardFeishu({
                            ...
                            replyInThread,
                            accountId,
                            mentions: sendableMentionTargets,
                        });
                    }
                    catch (err) {
    ...
    return {
        ...
        abortCard,
        getCompletedReplyText: () => deliveredReplyTexts.join('\n\n').trim(),
        getCompletedHandoffText: () => deliveredHandoffTexts.join('\n\n').trim(),
        getCompletedMentionTargets: () => [...deliveredMentionTargets.values()],
    };
}
